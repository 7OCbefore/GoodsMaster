...
export function useStore() {
  ...
  const loadData = async (userId = DEFAULT_USER_ID) => {
    ...
    try {
      ...
      const [packagesData, goodsData, salesData, pricesData] = await Promise.all([
        getPackages(userId),
        getGoodsList(userId),
      ...
      if (goodsData.error) throw goodsData.error;
      if (salesData.error) throw salesData.error;
      if (pricesData.error) throw pricesData.error;
      
      packages.value = packagesData.data || [];
      goodsList.value = goodsData.data || [];
      salesHistory.value = salesData.data || [];
      
      // Convert prices array to object for easier lookup
      sellPrice.value = (pricesData.data || []).reduce((acc, price) => {
